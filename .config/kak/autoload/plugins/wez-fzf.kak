...
define-command _wez-fzf -hidden -params 1.. %{
  ...
  nop %sh{
    ...
    pick() {
      ...
      ([ -n "$git_base_dir" ] && [ -z "$dir" ]) && dir="$git_base_dir"
    ...
    global_search() {
      ...
      run_command="cd $dir"
      run_command="$run_command; pane_id=\$(wezterm cli get-pane-direction up)"
      run_command="$run_command; echo -en \":change-directory \"$dir\"\r\" | wezterm cli send-text --no-paste --pane-id \$pane_id"
      run_command="$run_command; selection=\$(
        $grep_command |                                                               \
        fzf -e --delimiter=':' --preview '                                            \
           LINE={2};                                                                  \
           if [ \$LINE -lt 16 ]; then                                                 \
             START=1;                                                                 \
           else                                                                       \
    ...
    main() {
      local fpath="${2:-"."}"
      local cwd="${3:-"."}"
      local buffer_dir=$(dirname "$fpath")

      local git_base_dir="$(git rev-parse --show-toplevel 2>/dev/null)"

      local run_command

      case "$1" in
        buffer) run_command="$(buffer)" ;;
        pick) run_command="$(pick "$git_base_dir")" ;;
        pick_cwd) run_command="$(pick "$git_base_dir" "$cwd")" ;;
        pick_cbd) run_command="$(pick "$git_base_dir" "$buffer_dir")" ;;
        global_search) run_command="$(global_search "$git_base_dir" "$git_base_dir")" ;;
        *) exit 1 ;;
      esac

      local pane_id=$(wezterm cli get-pane-direction down)
