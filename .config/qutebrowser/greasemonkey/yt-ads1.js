...
(function () {
  ...
  function skipVideoAds() {
    ...
    const isAdPlaying =
      ...
      document.querySelector(
        ".video-ads, .ytp-ad-module, .ytp-ad-text, .ytp-ad-preview",
      );
    if (!isAdPlaying) {
      return;
    }
    ...
    if (video && video.duration) {
      const isLikelyAd =
        video.duration < 60 ||
        document.querySelector(
          ".ytp-ad-text, .ytp-ad-skip-button-container, .ytp-ad-preview",
        );
    ...
    if (video && video.paused && isLikelyAd) {
      video
        .play()
        .catch(() => console.log("Erreur lors de la reprise après publicité"));
    }
  }
  ...
  function removeAdBanners() {
    ...
    relatedItems.forEach((item) => {
      if (
        item.querySelector(
          'ytd-ad-slot-renderer, [class*="ad"], [class*="sponsored"]',
        ) &&
        item.parentNode
      ) {
  ...
  function bypassAgeRestriction() {
    ...
    const ageRestrictionMessage = document.querySelector(
    ...
    if (ageRestrictionMessage && currentUrl.includes("watch?v=")) {
      ...
      if (video && video.paused) {
        video
          .play()
          .catch(() => console.log("Erreur lors de la tentative de lecture"));
      }
    }
  }
  ...
  function handleAdsAndAgeRestrictions() {
    try {
      ...
      console.log(
        "Erreur lors de la gestion des publicités ou restrictions d'âge:",
        error,
      );
    }
  }
