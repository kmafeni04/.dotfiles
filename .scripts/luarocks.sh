_luarocks() {
    ...
    opts="-h --help --version --dev --server --from --only-server --only-from --only-sources --only-sources-from --namespace --lua-dir --lua-version --tree --to --local --global --no-project --force-lock --verbose --timeout --project-tree --pack-binary-rock --branch --sign"
    ...
    for arg in "${args[@]}"; do
        case "$arg" in
            ...
            build)
                cmd="build"
                opts="$opts -h --help --only-deps --deps-only --branch --pin --no-install --no-doc --pack-binary-rock --keep --force --force-fast --verify --sign --check-lua-versions --pin --no-manifest --only-deps --deps-only --deps-mode --nodeps"
                break
                ;;
            config)
                cmd="config"
            ...
            init)
                cmd="init"
                opts="$opts -h --help --wrapper-dir --reset --no-wrapper-scripts --no-gitignore --output --license --summary --detailed --homepage --lua-versions --rockspec-format --tag --lib"
                break
                ;;
            install)
                cmd="install"
            ...
            path)
                cmd="path"
                opts="$opts -h --help --no-bin --append --lr-path --lr-cpath --full --lr-bin --bin"
                break
                ;;
            purge)
                cmd="purge"
